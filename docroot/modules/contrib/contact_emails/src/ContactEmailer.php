  ...
  public function sendEmails() {
    ...
    if ($contact_emails) {
      foreach ($contact_emails as $email) {
        // If we using the 'context' recipient_type then this contact_message
        // becomes as 'personal' bundle for the contexted user.
        if ('context' == $email->get('recipient_type')->getString() &&
          $this->contactMessage->get('recipient')->isEmpty() &&
          $author = $email->getContextEntityAuthor(TRUE)
        ) {
          $this->contactMessage->set('recipient', $author)->save();
        }
        ...
        if ($email->hasTranslation($current_lang)) {
          $email = $email->getTranslation($current_lang);
        }
