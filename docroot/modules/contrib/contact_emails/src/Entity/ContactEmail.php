...
class ContactEmail extends ContentEntityBase implements ContactEmailInterface {
  ...
  public function getFormat(MessageInterface $message) {
    ...
    // Default to html.
    $format = 'text/html';
    if (\Drupal::config('contact_emails.settings')->get('allow_charset_utf_8')) {
  ...
  public function getRecipients(MessageInterface $message) {
    ...
    switch ($type) {
      ...
      case 'context':
        $recipients = $this->getContextEntityAuthor();
        break;
  ...
  public function getReplyTo(MessageInterface $message) {
    ...
    switch ($type) {
      ...
      case 'context':
        $reply_to = $this->getContextEntityAuthor();
        break;
   ...
   * {@inheritdoc}
   */
  public function getContextEntityAuthor($return_owner = FALSE) {
    $owner = NULL;
    $mails = [];

    foreach (\Drupal::routeMatch()->getParameters() as $parameter) {
      /** @var \Drupal\user\UserInterface $owner */
      if (is_object($parameter) && $parameter instanceof EntityOwnerInterface && $owner = $parameter->getOwner()) {
        $mails[] = $owner->getEmail();
      }
    }

    return $return_owner ? $owner : array_filter($mails);
  }
  ...
  protected function getEmailFromField(MessageInterface $message, $field) {
    ...
    if ($message->hasField($field)) {
      $instance = $message->get($field);
      $definition = $instance->getFieldDefinition();
      ...
      if ($emails) {
        foreach ($emails as $email) {
          if ($email['value']) {
            if ($type == 'contact_storage_options_email') {
              $results = array_merge($results, explode(',', $allowed_values[$email['value']]['emails']));
            }
            else {
              $results[] = $email['value'];
            }
