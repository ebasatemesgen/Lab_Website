...
class ContactEmailForm extends ContentEntityForm {
   ...
   *   The route match.
   */
  public function __construct(EntityRepositoryInterface $entity_repository, EntityTypeBundleInfoInterface $entity_type_bundle_info, TimeInterface $time, ContactEmails $contact_emails, EntityTypeManagerInterface $entity_type_manager, RouteMatchInterface $route_match) {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['recipient_type']['widget'][0]['value'] = array_merge($form['recipient_type']['widget'][0]['value'], [
      ...
      '#options' => [
        ...
        'context' => $this->t("The context entity author's email"),
  ...
  protected function getContactForm(FormStateInterface $form_state) {
    ...
    if (!$entity->get('contact_form')->isEmpty()) {
      $contact_form = $entity->get('contact_form')->target_id;
    }

    if (empty($contact_form) && $form_state->hasValue('contact_form')) {
      $value = $form_state->getValue('contact_form', NULL);
      $contact_form = is_array($value) ? $value[0]['target_id'] : NULL;
    }

    /** @var \Drupal\contact\ContactFormInterface $contact_form */
    if (empty($contact_form)) {
      $contact_form = $this->routeMatch->getParameter('contact_form');
    }

    if (is_string($contact_form)) {
      $contact_form = ContactForm::load($contact_form);
    }
  ...
  public function save(array $form, FormStateInterface $form_state) {
    $save = parent::save($form, $form_state);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $storage = $this->entityTypeManager->getStorage('contact_email');
