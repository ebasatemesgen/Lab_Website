...
class DropzoneJs extends FormElement {
  ...
  public static function processDropzoneJs(&$element, FormStateInterface $form_state, &$complete_form) {
    ...
    if (!empty($element['#chunking'])) {
      $generated_url_upload = Url::fromRoute('dropzonejs.chunked.upload')->toString();
      $generated_url_finalize = Url::fromRoute('dropzonejs.chunked.finalize')->toString();

      $element['uploaded_files']['#attributes'] = [
        'data-upload-path' => $generated_url_upload,
        'data-finalize-path' => $generated_url_finalize,
      ];
    }
    else {
      $element['uploaded_files']['#attributes'] = [
        'data-upload-path' => $generated_url->getGeneratedUrl(),
      ];
    }
